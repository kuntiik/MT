...
from mt.core import RecordType
from mt.data import transform_dl
import mt.models.mmdet.common
from mt.models.mmdet.common.utils import convert_background_from_zero_to_last
...
def _labels(record):
    if len(record.detection.label_ids) == 0:
        ...
        tensor_label_ids = torch.tensor(record.detection.label_ids)
...
def _bboxes(record):
    if len(record.detection.label_ids) == 0:
        ...
        return torch.tensor(xyxys, dtype=torch.float32)
